...
#define    PVT_POINT_BUFFER_SIZE      32


#if FPGA_CALC_PVT
    #if DATA_USE_DOUBLE
    #define    OUTPUT_DATA_BUFFER_SIZE    12            //加速算法里面，代表的是段数(加速段、匀速段或者减速段)
    #else
    #define    OUTPUT_DATA_BUFFER_SIZE    18            //加速算法里面，代表的是段数(加速段、匀速段或者减速段)
    #endif

#else
    #if GELGOOG_AXIS_4 ||  QUBELEY_HARDVER_1
    #define    OUTPUT_DATA_BUFFER_SIZE    4095          //波表Buffer的长度，值要是2^n - 1，最大4095(16384个字节)

    #elif GELGOOG_SINANJU
    #define    OUTPUT_DATA_BUFFER_SIZE    2047          //波表Buffer的长度，值要是2^n - 1，最大4095(16384个字节)
...
#define    EVENT_CODE_LEN      6     
#define    SOFTWARE_VER_LEN    5
#define    FPGA_VER_LEN        6
...
#define    LED_FLICKER_FREQ_MIN   100
#endif
...
#define    PDM_MSTEP_BUFFER_SIZE    508   //504+4(4分为两部分，2个字节寄存器地址长度和2个保留长度)

#define    MRV_CH_TOTAL_NUM      1

#define    DEFAULT_SCALE_NUM    250
...
typedef enum
{
    SDMODEL_C23D = 0,   //单轴，03/1.0版，262
    SDMODEL_C23S,       //单轴，03/1.1版，262
    
    SDMODEL_M2304,      //4轴，262
    SDMODEL_MV,         //机械臂5轴，262
    SDMODEL_M1710,      //10轴
    
    SDMODEL_M2302,      //2轴，四轴基础上阉割出来的2轴
    SDMODEL_M23041,      //4轴，262
    SDMODEL_MV1,         //机械臂5轴，262
...
typedef enum
{
    ...
    REPTTYPE_DRVSGALL,  //SGALL获取    //2
    REPTTYPE_DRVSGSE,   //SGSE获取
    
    REPTTYPE_DISTANCE,  //测距传感器值    //4
    REPTTYPE_ABSENC,    //绝对值编码器
    
    REPTTYPE_OUTNUM,        //失步数             //6
...
{
    UARTNUM_U1 = 0,
    UARTNUM_U2,    //实际上只有单轴才支持U2，但是这里没有屏蔽是为了和让机械臂参数向前兼容，当然4轴和10轴就不兼容了
